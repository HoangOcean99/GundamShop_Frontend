        ...
        <aside>
          <div className="bg-[#0A0A0E]/80 backdrop-blur-2xl border border-white/5 p-6 rounded-sm shadow-2xl relative overflow-hidden group h-full">
            ...
            <div className="flex items-center space-x-4 border-b border-white/5 pb-10 mb-8 mt-4">
              <div className="relative">
                <div className="w-14 h-14 rounded-sm bg-gradient-to-tr from-blue-600/20 to-indigo-900/20 border border-white/10 flex items-center justify-center shadow-[0_0_20px_rgba(0,102,255,0.2)]">
                  {avatar ? (
                    <img src={avatar} alt="avatar" className="w-12 h-12 object-cover rounded-sm" />
                  ) : (
              ...
              <div className="overflow-hidden">
                ...
                <div className="flex items-center mt-1">
                  <span className="text-[8px] font-black tracking-widest text-blue-400 uppercase bg-blue-400/10 px-1.5 py-0.5 rounded-sm">Verified</span>
                </div>
              </div>
            </div>
            ...
            <nav className="space-y-3">
              {[
                { id: 'profile', name: 'Dữ liệu cá nhân', icon: <IoPersonOutline /> },
                { id: 'orders', name: 'Lịch sử nhiệm vụ', icon: <IoBagCheckOutline /> },
              ].map((item) => (
                <div
                  key={item.id}
                ...
                >
                  <div className="flex items-center">
                    <span className={`text-lg mr-4 transition-transform duration-500 ${view === item.id ? 'scale-110' : 'group-hover/nav:scale-110 opacity-50'}`}>
                      {item.icon}
                    </span>
          ...
          {view === 'profile' ? (
            <div className="bg-[#0A0A0E]/80 backdrop-blur-2xl border border-white/5 p-10 rounded-sm shadow-2xl relative overflow-hidden h-full">
              ...
              <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center">
                      <IoPersonOutline className="mr-2 text-blue-400" /> Tên hiển thị
                    </label>
                    <input
                      type="text"
                      name="name"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center">
                      <IoPhonePortraitOutline className="mr-2 text-blue-400" /> Hệ thống liên lạc
                    </label>
                    <input
                      type="tel"
                      name="phone"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center">
                    <IoLocationOutline className="mr-2 text-blue-400" /> Điểm tập kết (Địa chỉ)
                  </label>
                  <input
                    type="text"
                    name="address"
            ...
            <div className="bg-[#0A0A0E]/80 backdrop-blur-2xl border border-white/5 p-10 rounded-sm shadow-2xl relative overflow-hidden h-full flex flex-col">
              <div className="flex items-center justify-between mb-12 border-b border-white/5 pb-6">
                ...
                <div className="text-[10px] font-black text-gray-500 tracking-[0.2em] uppercase">Total Logs: {orders.length}</div>
              </div>

              {loadingOrders ? (
                <div className="flex-grow flex items-center justify-center py-20"><LoadingSpinner size="lg" /></div>
              ) : orders.length === 0 ? (
                <div className="flex-grow flex flex-col items-center justify-center py-20 text-gray-600">
                  ...
                  <p className="font-black italic tracking-widest uppercase text-sm">Chưa có nhiệm vụ nào được ghi nhận</p>
                </div>
              ) : (
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                          <div className="space-y-4 flex-1">
                            <div className="flex items-center space-x-4">
                              <span className="text-[10px] font-black text-gray-500 tracking-widest uppercase">LOG-ID: {order._id.slice(-8).toUpperCase()}</span>
                              <div className={`flex items-center px-2 py-0.5 rounded-sm border text-[8px] font-black uppercase tracking-widest ${status.color}`}>
                            ...
                            <div className="flex items-center space-x-6">
                              <div className="flex -space-x-3">
                                {order.items.slice(0, 3).map((item, idx) => (
                                  <div key={idx} className="w-12 h-12 bg-black border border-white/10 rounded-sm overflow-hidden shadow-xl">
                          ...
                          <div className="flex flex-col items-end justify-between border-l border-white/5 pl-6 min-w-[150px]">
                            <span className="text-lg font-black text-blue-500 italic drop-shadow-[0_0_10px_rgba(0,102,255,0.2)]">{formatPrice(order.totalPrice)}</span>
          ...
          <div className="relative bg-[#0A0A0E] border border-white/10 w-full max-w-2xl h-[80vh] flex flex-col rounded-sm shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden">
             ...
             <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
               <div className="flex items-center space-x-4">
                 ...
                 <div>
                    <h3 className="text-sm font-black text-white uppercase tracking-[0.2em]">CHI TIẾT NHIỆM VỤ</h3>
             ...
             <div className="flex-grow overflow-y-auto p-8 custom-scrollbar space-y-8">
               ...
               <div className="grid grid-cols-2 gap-8 text-left">
                  <div className="space-y-2">
                    <span className="text-[8px] font-black text-gray-600 uppercase tracking-[0.2em]">TRẠNG THÁI HIỆN TẠI</span>
                    <div className="flex">
                      <div className={`flex items-center px-3 py-1 bg-white/5 border text-[10px] font-black uppercase tracking-widest rounded-sm ${getStatusInfo(selectedOrder.status).color}`}>
               ...
               <div className="space-y-3 pt-6 border-t border-white/5 text-left">
                  <span className="text-[8px] font-black text-gray-600 uppercase tracking-[0.2em]">THÔNG TIN GIAO HÀNG</span>
                  <div className="bg-white/5 p-4 rounded-sm border border-white/5">
                    <p className="text-xs font-bold text-gray-200 uppercase leading-relaxed">{selectedOrder.shippingAddress}</p>
                    <div className="flex items-center mt-3 text-[10px] font-black text-blue-400 uppercase tracking-widest">
                       <IoCardOutline className="mr-2" /> {selectedOrder.paymentMethod || "COD"}
                    </div>
                  </div>
               </div>
               ...
               <div className="space-y-4 pt-6 text-left">
                  <span className="text-[8px] font-black text-gray-600 uppercase tracking-[0.2em]">DANH SÁCH SẢN PHẨM ({selectedOrder.items.length})</span>
                  <div className="space-y-3">
                    {selectedOrder.items.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-white/[0.03] border border-white/5 rounded-sm group hover:bg-white/[0.05] transition-all">
                        <div className="flex items-center space-x-4">
             ...
             <div className="p-8 bg-white/[0.02] border-t border-white/5">
               <div className="flex justify-between items-end">
                  <div className="space-y-1 text-left">
                    <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">TỔNG HỢP CHI PHÍ</span>
                    <div className="flex items-center text-emerald-400 text-[10px] font-black">
                      <IoShieldCheckmarkOutline className="mr-1.5" /> SECURE TRANSACTION
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-gray-500 uppercase block mb-1">TOTAL PAYMENT</span>
                    <span className="text-3xl font-black text-blue-500 italic drop-shadow-[0_0_15px_rgba(0,102,255,0.4)]">{formatPrice(selectedOrder.totalPrice)}</span>
                  </div>
               </div>
             </div>
          </div>
