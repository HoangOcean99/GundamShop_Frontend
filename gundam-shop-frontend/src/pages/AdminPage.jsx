      ...
      <main className="flex-grow w-full grid grid-cols-[250px_1fr] relative z-20">
        ...
        <div className="p-10 h-[calc(100vh-73px)] overflow-y-auto">
          ...
          <div className="flex justify-between items-end mb-8">
            <div>
              <h1 className={`text-3xl font-black italic tracking-wider uppercase drop-shadow-lg ${activeTab === 'products-japan' ? 'text-blue-400' :
                ...
                <tbody className="divide-y divide-white/5">
                  {sortedProducts.map((product) => (
                    <tr key={product._id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-4 text-xs font-bold text-gray-500 text-center">#{product.code || product._id.slice(-6)}</td>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white/5 border-b border-white/10 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                    ...
                    <th className="p-4 w-32 text-center">Vai trò</th>
                    <th className="p-4 w-40 text-center">Ngày tạo</th>
                    <th className="p-4 w-32 text-center">Hành Động</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {users.map((user) => (
                    <tr key={user._id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-4 text-xs font-bold text-gray-500 text-center">#{user._id.slice(-6)}</td>
                      <td className="p-4 text-sm font-bold text-white tracking-widest">
                        {user.name || "N/A"}
                      </td>
                      ...
                      <td className="p-4 text-sm font-bold text-gray-300 text-center">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white/5 border-b border-white/10 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                    ...
                    <th className="p-4 w-48 text-right">Tổng tiền</th>
                    <th className="p-4 w-32 text-center">Trạng thái</th>
                    <th className="p-4 w-40 text-center">Ngày đặt</th>
                    <th className="p-4 w-32 text-center">Hành Động</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {orders.map((order) => (
                    <tr key={order._id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-4 text-xs font-bold text-gray-500 text-center">#{order._id.slice(-6)}</td>
                      <td className="p-4 text-sm font-bold text-white tracking-widest">
                        {order.user?.name || "N/A"}
                      </td>
                      <td className="p-4 text-sm font-bold text-gray-300 text-right">
